...
module CricketQuestions
  ...
  def pick_player(type, bb,  min_mat, date=Date.new(1877, 1, 1), team="")
    file=File.new("./#{type}/#{bb}mat.csv", "r")
    file.gets
    ar=[]
    while (line=file.gets)
      ...
      if to_date(ind[3]) < date && rm!=true
        ar.slice!(ar.length-1)
      end
    end
    ar.sample
  end
  def pick_matches(type, team, bb, pname, matches)
    file=File.new("./#{type}/#{team}/#{bb}/#{pname}.csv")
  ...
  class CrBatAverage < QuestionWithExplanation 
    def initialize
      nomat=rand(7)+2
      type="Test"
      pname=pick_player(type, "bat", nomat)
      @pname=pname
      puts @pname
      @mat=pick_matches(type, pname[0], "bat", pname[1], nomat)
    ...
    def text
      ...
      beg=to_date(@mat[0][@names.index("Date")])
      en=to_date(@mat[@mat.length-1][@names.index("Date")]).+(6)
  ...
  class CrBowlAverage < QuestionWithExplanation 
    def initialize
      nomat=rand(4)+5
      type="Test"
      pname=pick_player(type, "bowl", nomat)
      @pname=pname
      puts @pname
      @mat=pick_matches(type, pname[0], "bowl", pname[1], nomat)
      @names=@mat.slice!(0)
      tr=false
      for i in 0...@mat.length
        puts @mat[i]
  ...
  class CrStrikeRate
    def initialize
      nomat=rand(7)+2
      type="Test"
      pname=pick_player(type, "bat", nomat, EARLIEST)
      @pname=pname
      puts @pname
      @mat=pick_matches(type, pname[0], "bat", pname[1], nomat)
      @names=@mat.slice!(0)   
  ...
  class CrStrikeRateBowling
    def initialize
      nomat=rand(4)+5
      type="Test"
      pname=pick_player(type, "bowl", nomat, EARLIEST)
      @pname=pname
      puts @pname
      @mat=pick_matches(type, pname[0], "bowl", pname[1], nomat)
      @names=@mat.slice!(0)
